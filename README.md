...
### 3) 초기 데이터
`resources/data.sql`에 멤버십 정책 등급/구간/적립률에 대한 초기 데이터가 포함되어 있습니다.
...
- 이 프로젝트는 결제/구독/적립 도메인을 학습 및 과제 형태로 구현한 구조를 갖습니다.
- 실제 운영 환경에서는 결제 Webhook 검증(서명 검증), 멱등성 키, 트랜잭션 경계, 재시도/보상 처리 등을 강화하는 구성이 필요합니다.
